  ...
  'hover浮层': function hover(browser) {
    ...
    hoverList.forEach(function (item) {
      ...
      if (item.scrollElement) {
        (0, _tools.scrollToElement)(browser, item.scrollElement);
      }

      (0, _tools.handleHover)(browser, item.selector, item.targetSelector, item.name);
    });
    browser.end();
  },
  'click事件': function click(browser) {
    ...
    clickList.forEach(function (item) {
      ...
      if (selectorType === 'root') {
        (0, _tools.handleRootClick)(browser, item.selector, item.targetUrl, item.name);
        return;
      }

      if (item.search) {
        (0, _tools.handleSearch)(browser, item.selector, item.search.value, item.search.buttonSelector, item.targetUrl, test.url, item.name);
        return;
      }

      if (item.hover) {
        (0, _tools.handleHover)(browser, item.hover.selector, item.hover.targetSelector, item.hover.name, 1000); //待修正

        (0, _tools.handleBlankClick)(browser, item.selector, item.targetUrl, item.name);
        return;
      }

      if (item.target === 'self') {
        (0, _tools.handleSelfClick)(browser, item.selector, item.targetUrl, test.url, item.name);
        return;
      }

      if (item.target === 'element') {
        (0, _tools.checkBackToElement)(browser, item.selector, item.targetSelector, item.name);
        return;
      }
    ...
    // /* -----------  JOY寻宝  ----------- */
    // handleHover(browser,'//div[@class="w"]/div[@class="logo"]','//a[@class="logo_scene logo_scene_animateend logo_scene_withBg "]','JOY寻宝hover',1000) //待修正
    ...
    // /* -----------  今日推荐     ----------- */
    // handleRootClick(browser,'.recommend-item.fade-in',"jd",'点击“今日推荐”')  //待修正
    // /* -----------  购物菜单栏     ----------- */
